...
const bancoPreguntas = {
  "Colecciones y datos": [
    ...
    },
    {
      ...
      opciones: [
        "Toma elementos en posición par",
        "Toma elementos en posición impar",
        "Revierte la secuencia",
        "No hace nada",
      ],
      respuesta: 2,
    },
...
async function cargarJugadores() {
  try {
    ...
    state.jugadores = Array.isArray(arr)
      ? arr.map((p) => p.name || p.nombre || p)
      : [];
  } catch {
    state.jugadores = [];
  }
...
function mostrarPantallaJuego() {
  ...
  const msg = document.getElementById("mensajeSinJugadores");
...
function tirarRuleta() {
  ...
  // mostrar categoría elegida sobre la ruleta
  setTimeout(() => {
    if (etiquetaCat) {
      etiquetaCat.textContent = `Categoría: ${cat}`;
    }
    if (overlay) {
      overlay.textContent = `Categoría: ${cat}`;
...
function mostrarPregunta(categoria) {
  ...
  if (texto) texto.textContent = q.texto;
  if (opciones) {
    opciones.innerHTML = "";
...
function evaluarRespuesta(indice) {
  const opciones = document.querySelectorAll("#opciones .btn-menu");
...
function pasarAlSiguienteTurno() {
  ...
  if (pantallaPregunta) pantallaPregunta.classList.add("hidden");
  if (pantallaRuleta) pantallaRuleta.classList.remove("hidden");
  ...
  if (resultado) resultado.textContent = "";
  if (timer) timer.textContent = "Tiempo restante: 15";
  ...
  if (state.jugadores.length > 0) {
    state.indiceJugadorActual =
      (state.indiceJugadorActual + 1) % state.jugadores.length;
  }
  actualizarTurnoTexto();
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (ul) {
    state.categoriasOrden.forEach((cat, i) => {
      ...
      dot.style.background =
        state.colores[i % state.colores.length] || state.colores[0];
  ...
  if (btnTirar) {
    btnTirar.addEventListener("click", tirarRuleta);
  }
